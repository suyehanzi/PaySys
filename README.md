...
PaySys 是一个轻量订阅中转后台。它用你的上游 LILISI 账号刷新并缓存订阅内容，再给客户提供你自己的稳定订阅地址，同时管理客户 QQ、群名、到期时间、续费和禁用状态。

当前主流程：

1. 管理员在 `/admin` 新增客户，登记 QQ、群名、备注和续费。
2. 客户打开 `/portal`，输入已登记 QQ 号进入订阅中心。
3. 客户点击“获取订阅”，页面显示 `/sub/[token]` 订阅链接和二维码。
4. Clash、Singbox、Hiddify 等客户端请求 `/sub/[token]`，只有有效客户才能拿到缓存订阅内容。
...
- 客户管理：昵称、QQ、群名、备注、到期时间、禁用状态；客户列表里可直接编辑备注，离开输入框自动保存。
- 群名选择：新增客户时默认支持 `1群`、`2群`。
...
- 上游刷新：支持 LILISI API 自动登录获取 `subscribe_url` 并缓存内容。
...
1. 管理员打开 `/admin`。
2. 点“新增客户”，填昵称、QQ、群名、备注。
3. 后续需要补充或修改备注时，直接在客户列表的备注框里编辑，离开输入框后自动保存。
4. 客户付款后，在客户列表里登记续费，默认 `45` 元、`180` 天。
5. 点“复制文案”，把文案发到 QQ。
6. 客户打开 `/portal`，输入 QQ，点击“获取订阅”。
7. 客户复制 `/sub/[token]` 或扫码导入客户端。

客户过期或禁用后，`/sub/[token]` 会返回错误，不返回真实订阅内容。

## 后台记录

后台会保留客户获取订阅相关记录。客户列表里显示汇总次数和最近获取时间；页面下方的“最近获取记录”显示更完整的明细。
